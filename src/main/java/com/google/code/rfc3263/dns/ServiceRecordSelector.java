    ...
    if (services.size() == 1) {
      ...
      for (Entry<Integer, List<SRVRecord>> entry : priorityMap.entrySet()) {
        ...
        if (priorityList.size() != 1) {
          if (LOGGER.isDebugEnabled()) {
            LOGGER.debug("Multiple SRV records found at priority " + priority + ", using "
                         + weightingPrioritizer.getClass() + " as sorting algorithm");
          }
          weightingPrioritizer.prioritize(priorityList);
